...
def runFileMerge(folder):
        print(f" Running night: {folder}")
        # Run ./app with arguments

        os.chdir('/storage/hive/project/phy-otte/shared/Trinity/DataAnalysis/DataCalibration/MergedData/')
        # result =subprocess.run(["./FileMerge", folder],capture_output=True, text=True)
        cmd = ["sbatch", "-J", f"mf{folder[2:]}", "merge.sbatch", folder]
