...
def main():
    ...
    log_print('------- Starting Trigger Scan Mapping Testing ------') 
    ...
    try:
        for m in MUSIC_OFF: # goes between top and bottom     
            for s in SIAB: # goes through each SIAB
                ...
                run_expect('trigger_scan_special.exp', 'running RemoteControl TriggerScanSF -s 0 -ns 31 -sz 10 -t 5 trigger rate scan')
                log_print('Check Channel counts and Trig scan info\nHV ON')
                input("Hit \"ENTER\" to continue: ...")
                ...
                while True: 
                    ...
                    if key == 'c':  # Check if Enter key is pressed
                        ...
                        log_print("\nTrigger Scan Mapping Testing terminated by user. \n Make sure to shut the camera off\n")
                        sys.exit(0)
    ...
    print('------------------------------------------------------')
    log_print("\nTrigger Scan Mapping Testing is Complete. \n Make sure to shut the camera off\n")
##################################################################################################################################
##################################################################################################################################
################################################################################################################################## 
...
def trigger_scan(s):
    ...
    if t == 'y':
        ...
        run_expect(f"SIAB_threshold.exp {t} {s} {s}",f'set rate to {t}')
        return t
