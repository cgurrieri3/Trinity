...
def main():
    ...
    log_print('------- Starting Hardware Pixel Mapping Testing ------') 
    ...
    try:
        while True:
            ...
            run_expect(f"SIAB_threshold.exp {tRate} 0 15",f'set rate to {tRate}')
            ...
            while True: 
                ...
                if key == 'c':  # Check if Enter key is pressed
                    ...
                    log_print("\nHardware Pixel Mapping Testing terminated by user. \n Make sure to shut the camera off\n")
                    sys.exit(0)
...
def first_loop(loop,trigrate):
    if loop == 0: 
        t = input('Do you need to take a TriggerScan? y/value: ')
        if t == 'y':
            print("do trigger rate scan and then set threshold")
            #tRate=get_new_tRate()
            tRate = 145
            run_expect("conf_SFO.exp", "config sequence SFO")
            #time.sleep(20)
            ...
            run_expect(f"SIAB_threshold.exp {tRate} 0 15",f'set rate to {tRate}')
            tRate = 150
            print(f'rate set {tRate}')
            return tRate

        elif isinstance(int(t), int):
            run_expect(f"SIAB_threshold.exp {t} 0 15",f'set rate to {t}')
            return t
        ...
        #print(f'reserting threshold to {trigrate}')
        run_expect(f"SIAB_threshold.exp {trigrate} 0 15",f'set rate to {trigrate}')
        return trigrate
...
if __name__ == "__main__":
    # Configure logging
    log_file = 'PixelMappingTesting.log'
