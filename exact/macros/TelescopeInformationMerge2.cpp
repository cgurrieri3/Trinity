...
int runFile (std::string date, int option) {
	...
	std::string fileDir = "/storage/hive/project/phy-otte/shared/Trinity/Data/" + date;
	...
	if(option == 0){
		...
		char* fileNameDir = fileDir.c_str();
			
                void *dirp = gSystem->OpenDirectory(fileNameDir);
                const char *entry;
		while ((entry = gSystem->GetDirEntry(dirp))) {
			TString filePath = TString::Format("%s/%s", fileNameDir, entry);
			TelescopeInformationMerge1(extraFile, StateMessageFile, filePath, fileOut);
		}
	}
