...
int TelescopeInformationMerge1 (std::string extraFile, std::string StateMesssageFile,std::string filename, std::string fileOut){
	...
	std::string sunmoonfilename = Form("/storage/hive/project/phy-otte/shared/Trinity/DataAnalysis/DataCalibration/AncillaryData/Data1/%s", date.c_str());	
...
int runFile(std::string date, int option) {
	...
	if(option == 0){
		...
		while ((entry = gSystem->GetDirEntry(dirp))) {
			...
			if (yo == false){
			TString filePath = TString::Format("%s/%s", fileNameDir, entry);
			std::string strFilePath = filePath.Data();
			std::size_t start = strFilePath.find("CoBo0_AsAd0_")+12; // Find the position after "COBO_"
			std::string date = strFilePath.substr(start, 10);  // Extract "2024-08-07"
			yo = true;
			TelescopeInformationMerge1(extraFile, StateMessageFile, strFilePath, fileOut);
