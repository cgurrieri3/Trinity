...
void TCameraPlotEvents_Trinity_saveimg_stack_run(std::string filename, std::string orientation,std::string treeString)
{
...
// Extract the second-to-last directory
	...
	SetBranches(ev,&triggerTime, bf);
        ...
        for (int j = 0; j < MaxNofChannels; j++)
        {
            ...
            hcam_count->AddBinContent(hcam_count->GetBin(nx + 1, ny + 1), 0);
