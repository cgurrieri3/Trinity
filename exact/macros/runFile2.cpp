...
std::time_t findClosestIndex(const std::vector<double>& sortedVec, double value);
...
void TelescopeInformationMerge (std::string extraFile, std::string StateMesssageFile,std::string filename, std::string fileOut);
...
int runFile2(int predate, int option) {
	std::string date = std::to_string(predate);
	...
	//std::string filename = "/storage/hive/project/phy-otte/shared/Trinity/Data/20240718/RawDataMerged/CoBo0_AsAd0_2024-07-18T10:38:05.102_0000.root";
	std::string fileDir = "/storage/hive/project/phy-otte/shared/Trinity/Data/" + date + "/RawDataMerged";
	std::string fileOut = "/storage/hive/project/phy-otte/shared/Trinity/DataAnalysis/DataCalibration/MergedData";
	
	if(option == 0){
		const char* fileOutDir = fileOut.c_str();
			
		void *dirp = gSystem->OpenDirectory(fileOutDir);
		const char *entry;
		int filesDel = 0;
		while ((entry = gSystem->GetDirEntry(dirp))) {
			TString filePath = TString::Format("%s/%s", fileOutDir, entry);
			std::string stringfile = filePath.Data();	
			// Skip "." and ".." entries
			...
			if (std::remove(filePath) == 0) {
				std::cout << stringfile << " has been deleted succesfully. #" << filesDel <<  std::endl; 
			}
			else{
		...
		const char* fileNameDir = fileDir.c_str();
			
                void *dirp = gSystem->OpenDirectory(fileNameDir);
                const char *entry;
		...
		while ((entry = gSystem->GetDirEntry(dirp))) {
			...
			TString prefilePath = TString::Format("%s/%s", fileNameDir, entry);
			std::string filePath = std::string(prefilePath.Data());
			std::string fifileOut = fileOut + "/" +  entry;
			TelescopeInformationMerge(extraFile, StateMessageFile, filePath, fifileOut);
...
void TelescopeInformationMerge(std::string extraFile, std::string StateMesssageFile,std::string filename, std::string fileOut){
//int TelescopeInformationMerge (std::string extraFile, std::string StateMesssageFile,std::string filename, std::string name){
    ...
    for(int i = 0; i < tBiFocal->GetEntries(); i++){
        ...
        cout << "BiFocal Event# " << i <<" is being saved." << endl;
        ...
        // //iEvBiFocal->SetRunNumber(nRunBiFocal);
...
*/
    for(int i = 0; i < tForced->GetEntries(); i++){
        ...
        cout << "Forced Event# " << i <<" is being saved." << endl;
        ...
        // //iEvForced->SetRunNumber(nRunBiFocal);
        // //iEvForced->SetRevTimeTB(rTimeBiFocal);
        fOut->cd();
        tMForced->Fill();
	std::cout << "Hello World I am alive" << std::endl;
    }
    ...
    for(int i = 0; i < tTest->GetEntries(); i++){
        ...
        std::vector<float> value = WeatherTempature[closestIndex];
