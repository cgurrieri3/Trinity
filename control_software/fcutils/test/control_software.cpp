...
int main(int ac, char* av[])
{
	...
	std::string mqnames[2] = {"/CStoRC","/RCtoCS"};
	my_pmq_manager rq;
    rq.add_mq(mqnames[1]);
	rq.open();
	rq.print();
       	...
       	while(1){
    		...
    		if(!msg.empty()) {
				...
				//std::cout << " Writing the message :"<< msg << ", of length: "<< length << std::endl;
					if(length == 104){
						std::cout << "State Message size is correct" << std::endl;
						//std::cout << " Writing state message :" << std::hex << msg << ", of length: "<< length << std::endl;
					}else{
						msg[1] = 'C';
						std::cout << " State Message is corrupted" << std::endl;
					}
    				// Create the buffer sequence
    				status_msg[0]=ba::buffer(gccheader,GCCHEADER_SIZE);
					...
					if(umsg_id == 0xAA)
					{
						...
						while(std::getline(infile, line))
						{
							...
							if(command.substr(0,1) != "*"){
								wq.send(mqnames[0], command.c_str(), command.size());								
							}
							else{
    					...
    					wq.send(mqnames[0], umsg.c_str(),umsg.size());
					}
