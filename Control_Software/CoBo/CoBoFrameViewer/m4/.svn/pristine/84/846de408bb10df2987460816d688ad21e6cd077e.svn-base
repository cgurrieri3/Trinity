...
dnl Let users provide the location
	AC_ARG_WITH([qwt],
		[AC_HELP_STRING([--with-qwt], [user defined path to Qwt library])],
		[
			if test "$withval" = "no"
			then
				want_qwt="no"
...
dnl directories with spaces will not work.
		QT_CXXFLAGS=`echo $QT_CXXFLAGS | tr -d \" | sed 's/$(QT_DEFINES)/'"$QT_DEFINES"'/g'`
...
dnl Check for header usability
			...
			then
				...
				CXXFLAGS="${saved_CXXFLAGS} ${QT_CXXFLAGS}"
...
dnl Define Qwt library candidates based on Qt version
			AC_MSG_CHECKING([Qt version for Qwt])
			qmake_version_major_sed=['/^.*\([0-9]\)\.[0-9]\{1,\}\.[0-9]\{1,\}.*$/!d;s//\1/']
			ax_qwt_cv_QT_VERSION_MAJOR=`$QMAKE --version 2>&1 | sed "$qmake_version_major_sed"`
			AC_MSG_RESULT([$ax_qwt_cv_QT_VERSION_MAJOR])

dnl Check for Qwt library
			AC_MSG_CHECKING([for Qwt library])
			if test "$ax_qwt_cv_QT_VERSION_MAJOR" = "5";
			then
				ACX_PROG_SEARCH_LIB(
				[
					AC_LANG_SOURCE(
				...
				[qwt6-qt5 qwt5-qt5 qwt-qt5 qwt6 qwt5 qwt],
				[
					qwt_lib_found="yes"
				...
				[qwt6-qt4 qwt5-qt4 qwt-qt4 qwt6 qwt5 qwt],
				[
					qwt_lib_found="yes"
					QWT_LIBS="-l$found_lib"
				],
...
dnl Check for Qwt framework
			...
			then
				...
				frameworks = 'qwt6-qt4' 'qwt5-qt4' 'qwt-qt4' 'qwt6' 'qwt5' 'qwt'
				if test "$ax_qwt_cv_QT_VERSION_MAJOR" = "5";
				then
					frameworks = 'qwt6-qt5' 'qwt5-qt5' 'qwt-qt5' 'qwt6' 'qwt5' 'qwt'
				fi
				for qwt_framework in $frameworks
				do
