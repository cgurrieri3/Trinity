...
def trackTarget(startday, endday): 
    ...
    for date in dayList:
        ...
        for hors in hor_strict:
            ...
            ngc_targetWindow.append((trinity.next_setting(ngc, start = date)).datetime())
            ngc_arr_strict.append(ngc_setting.split()[1])
            txs_setting = str(trinity.next_setting(txs, start = date))
            txs_targetWindow.append((trinity.next_setting(txs, start = date)).datetime())
    ...
    if (numDays == 1) & (__name__ == '__main__'):
        return [ngc_opportunityWindow[0], ngc_opportunityWindow[1], txs_opportunityWindow[0], txs_opportunityWindow[1]]
    else:
        return ngc_targetWindow, txs_targetWindow

def main():
    trackTarget(dt.datetime(2023, 10, 1, 0, 0, 0), dt.datetime(2023, 10, 2, 0, 0, 0))
    return None
