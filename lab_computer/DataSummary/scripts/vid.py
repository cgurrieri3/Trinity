...
def make_vids_horizon(date,cam):
    ...
    os.system("mkdir -p {0}/{1}/jpg/{2} && cd {0}/{1}/jpg/ && ffmpeg -framerate 15 -pattern_type glob -i '{2}*.jpg' -c:v libx264 -pix_fmt yuv420p {1}_{2}.mp4 && /bin/mv {1}_{2}.mp4 {0}/VIDS".format(HOME, cam, date))
    os.chmod("{0}/VIDS/{1}_{2}.mp4".format(HOME,cam,date), 0o777)
