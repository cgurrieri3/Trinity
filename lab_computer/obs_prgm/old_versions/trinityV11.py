	...
	if safe_proceed == 1:
		...
		if safe_proceed == 1:
			...
			if safe_proceed == 1:
				...
				lets.fancy_communicate('Trigger Rate 170 \n SM enabled')
				lets.log_file('Trigger Rate set 170 ')
				...
				try:
					exit_mess=monitor_observations('e',wx_override = wx_override)
				except:
	...
	if command == 'fromDAQ':
		...
		if rclog == 1: 
			...
			if safe_proceed == 1:
				...
				# data completed or errror
				lets.fancy_communicate('Camera Shutting down...')
				lets.log_file('Camera shutting down')

				shut_down_CT()
				lets.fancy_communicate('Camera off')
				lets.log_file('Camera shutdown ')
				lets.send_email(exit_mess)
				lets.communicate(f'Email sent for reason {exit_mess} ')
			else:
...
def main():
	while True:
		...
		if com_in == 'off': # dont need to check the rc logs shuts down from any state
			...
			monitor_observations('i',runs,wx_override)
