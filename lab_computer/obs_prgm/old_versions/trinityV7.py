...
def internal_triggers(amount):
	...
	time.sleep(210) # change this to try try for 3 minutes every 20 seconds rather then wait incase it is sooner
	...
	while True:
		...
		if com_in == 'off':
			...
			if com_in == 'fromIntrigs' or com_in == 'fromDAQ' or com_in == 'newScan':
				...
				if len(list_trig) == 3:
					ssh.trigger_scan(com_in,start=start,step=step,size=size)
			elif com_in== 'startDAQ':
				rate = input('New trigger rate: ')
				#check that it is a number
				if rate.isdigit() == True:
					ssh.trigger_scan(com_in,rate=rate)
