...
def monitor_observations(state,intrigs_nfiles=10):
	intial_files = check_data_directory()
	print(intial_files)
	...
	# runs while observing to look for bad weather, state messages, or light levels to shut down the camera.
		...
		if state == 'i':
			...
			file_check_time = datetime.datetime.now()
			time_difference = file_check_time - current_time
			
			files=check_data_directory()
			# Checks if a file has been created in the first 6 minutes
			if files == intial_files and time_difference.total_seconds() > 360:
				print(f'CM: DA Internal Trigger files not being generated')
				break
		...
		if state == 'e':
			...
			updated_files=int(check_data_directory())
			if updated_files >= int(intial_files):
...
def internal_triggers(amount):
	...
	time.sleep(180) # change this to try try for 3 minutes every 20 seconds rather then wait incase it is sooner
	...
	if safe_proceed == 1:
		...
		monitor_observations('i',intrigs_nfiles=3)
	...
	if safe_proceed == 1:
		...
		if safe_proceed == 1:
			...
			if safe_proceed == 1:
				...
				# check info while taking data
				monitor_observations('e',intrigs_nfiles=10)
				# data completed or errror
